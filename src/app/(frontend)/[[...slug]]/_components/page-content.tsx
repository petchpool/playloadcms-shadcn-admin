...
/**
 * Async component that fetches and renders page content
 * This component is wrapped in Suspense to allow streaming
 */
export async function PageContent({ slug, searchParams }: PageContentProps) {
  // Skip static files
  const staticFiles = ['favicon.ico', 'robots.txt', 'sitemap.xml', '_next', 'api']
  if (slug?.[0] && staticFiles.includes(slug[0])) {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        ...
        <p className="mt-4 text-muted-foreground">
          ...
          <code className="px-2 py-1 bg-muted rounded">{pageSlug || 'home'}</code>
        </p>
